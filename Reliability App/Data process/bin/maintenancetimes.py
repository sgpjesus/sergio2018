...
def main_pipeline(path):
    r"""
    For a given maintenance oriented .csv file, with the column scheme given by
    specification, this function is capable of dividing total, complete and
     incomplete times in three different pandas.DataFrame.
    ...
    dates, failures = date_maint_lists(raw_data, earliest_time, latest_time,
                                       rows_machine, columns_component)

    total_difs, incomp_difs, comp_difs = calculate_time_difs(dates, failures)

    total_times_frame = pd.DataFrame(data=np.array(total_difs),
                                     columns=columns_component,
                                     index=rows_machine)
    incomp_times_frame = pd.DataFrame(data=np.array(incomp_difs),
                                      columns=columns_component,
                                      index=rows_machine)
    comp_times_frame = pd.DataFrame(data=np.array(comp_difs),
                                    columns=columns_component,
                                    index=rows_machine)
...
def col_row_identification(raw_data_frame):
    r"""
    For a given Dataframe, returns the unique values of the wanted rows and
    columns.
...
def date_maint_lists(raw_data_frame, start_time, end_time, machines, components):
    r"""
    For the data frame, removes in list form the dates and type of work order,
    retaining machine ID and component.
    ...
    for machine in machines:
        ...
        for comp in components:
            aux_list = [start_time] + \
                       list(raw_data_frame[raw_data_frame['machineID'] ==
                                           machine]
                            [raw_data_frame['comp'] == comp]['datetime']) + \
                       [end_time]
            aux_fail_list = list(raw_data_frame[raw_data_frame['machineID'] ==
                                                machine]
                                 [raw_data_frame['comp'] == comp]
                                 ['IF_FAIL']) + [0]
...
def calculate_time_difs(times_list, fails_list):
    r"""
    Calculates the time between two maintenance operations and divides it or not
    in three lists
    :param times_list: list of maintenance operations dates
    :param fails_list: list of maintenance operations type
    :return: three lists, consisting of times between maintenance with
    indiscriminate type, corrective and preventive
    """
